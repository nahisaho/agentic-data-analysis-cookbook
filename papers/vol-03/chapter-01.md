...
例：過去 5 年で 10,000 枚以上撮った顕微鏡画像がある。**ラベル付けは 200 枚**しかできていない。scikit-learn では 200 枚しか使えないが、実は残り 9,800 枚には **表現学習の材料が眠っている**。

- **vol-02 での限界**：教師なしは PCA/クラスタリング止まり。**「ラベルなしデータで表現を作り、少数のラベルで fine-tune する」枠組みがない**
...
CPU 上の scikit-learn / PyMC では、seed を固定すれば概ね bit 単位で再現できました。しかし GPU では：
...
vol-01 の Human-in-the-loop を、深層特有の場面に敷き詰めます。特に：

- 学習ジョブの起動（GPU 消費・時間・電気代を Human が承認）
- fine-tune の起動（特に「今日のバッチだけで再 fine-tune するか」）
- checkpoint の上書き（禁止 / 世代管理 / 明示承認あり）
- Foundation Model の更新受け入れ（上流が新版になったとき採用するか）
- 不確かさ超過時の自律停止（**エージェントは自律決定を止めて Human に投げる**）
- augmentation の追加（**精度を上げるために augmentation を勝手に増強しない**）

vol-03 では、Skill ごとに **`agent_authorization` の 3 段階**——**推論のみ / 承認済み範囲内の fine-tune 可 / 事前承認済みワークフロー内の自律実行可**——を宣言し、上記のゲートを配置します（第4章）。
...
**これらの課題は、深層学習の教科書には書かれません**。「エージェントが深層を回す」という前提を置いて初めて浮かび上がる論点です。ここが vol-03 の存在意義です。
...
- **主用途**：vol-02 第13章 capstone の深層版。**同じ合成階層データ**を、FM で特徴抽出 → PyMC 階層モデルで分散成分分解
...
2020 年代半ばの時点で、材料・ナノテク分野でも Foundation Model が急速に増えています（MatBERT、CrystaLLM、ChemBERTa、SciBERT、MoLFormer、CGCNN、M3GNet、MACE、UMA 等）。研究室が取りうる戦略は次の 4 択に整理できます。

| 選択肢 | 概要 | GPU 要件 | Human 承認負荷 | vol-03 での扱い |
|---|---|---|---|---|
| **選択肢 1**：FM を使わず、古典 ML + 教師あり深層で押す | vol-02 の scikit-learn + vol-03 第6章の教師あり深層 | 1 枚あれば十分 | 低 | 第6章の柱、多くの研究室で第一選択 |
| **選択肢 2**：公開 FM を frozen feature extractor として使う | FM の中間層出力を特徴量として scikit-learn / PyMC に流す | 推論のみ、CPU も可 | 中（重み provenance の管理が必要） | 第7章前半・第13章 capstone |
| **選択肢 3**：公開 FM を fine-tune する | LoRA / PEFT / full fine-tune で自データに適合 | 1〜数枚（LoRA なら小さめ） | 高（fine-tune 判断 + FM 更新受け入れ判断） | 第7章後半・第11章 |
| **選択肢 4**：自前で SSL / 事前学習する | ラベルなしデータで SimCLR / MoCo 等で表現学習 | 複数枚が現実的 | 高（学習ジョブ承認 + 重み配布判断） | 第12章、**選ぶべき場面は限られる** |

**多くの ARIM 会員研究室にとって、現実解は選択肢 1 + 2**、**特に少データ環境では選択肢 2 or 3**、**大量のラベルなしデータがあり装置固有性が強い場合のみ選択肢 4** です。vol-03 は 4 択すべてを扱いますが、**判断のしどころ**——「今、自分の研究室は選択肢 X をやるべきか？」——を各章の冒頭に置くことに重心を置きます。
...
| **強化学習** | 実験計画・自律実験に近いが、vol-03 のスコープ外 | vol-04 候補 |
| **深層 × 因果 × ベイズ最適化** | 各要素の熟成後に統合すべきトピック | vol-05 以降候補 |
...
### 「じゃあ ARIM で生成モデル・逆設計・因果推論が必要になったらどうする？」

現時点では、次の順序で進めることを推奨します：

1. **vol-03 の 2 pillars で表現学習と不確かさに慣れる**：生成モデルも逆設計も、表現学習と不確かさが土台
2. **vol-04（想定）で因果推論・ベイズ最適化に進む**：介入設計・実験計画・DAG
3. **vol-05 以降（想定）で生成モデル・逆設計に進む**：VAE / Diffusion / Flow / Bayesian optimization based inverse design
...
- [ ] 研究室が取るべき戦略（選択肢 1〜4）を、自分の状況で判断できる
...
- [第13章 合成階層データによる Advanced Capstone](../vol-02/chapter-13.md)（vol-03 capstone の元）
