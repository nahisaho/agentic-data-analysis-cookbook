...
| **Attribution の sanity check 通過** | Grad-CAM/IG が入力/パラメータランダム化に対して頑健 | model-parameter-randomization test を通過（第10章） |
...
>   - **予測時**：エージェント自律決定は **predictive entropy ≤ 0.3** の場合のみ、それ以外は Human 送り
...
第2章 §2.5 と第3章 §3.6 で挙げた provenance フィールドを、**3 レイヤ**に整理します。
...
| レベル | 名称 | エージェントに許すこと | 典型的な運用 |
|---|---|---|---|
| **L1** | **推論のみ** | 学習済みモデルの読み込み・推論・attribution 計算・レポート生成 | 本番運用、Human は結果を確認して意思決定 |
| **L2** | **承認済み範囲内 fine-tune 可** | L1 + Skill 契約に書かれた**範囲内**でのハイパーパラメータ選択・学習ジョブ起動・新 checkpoint 保存（**別名のみ**） | 週次の再学習パイプライン、契約範囲は Human が事前定義 |
| **L3** | **事前承認ワークフロー内自律実行可** | L2 + **既存 checkpoint 上書き**（ただしバージョンタグ付き）・複数実験の自律スケジュール | 実験計画自動化パイプライン（本書は L3 の設計まで、運用は読者環境） |
...
- **サンプル数**：想定 n の最小・最大
- **grouping key**（装置別・ロット別 CV 用、vol-02 第7章 → 第7章）
- **事前学習重み仕様**：`weights_uri` / `revision`(commit hash) / `sha256` / `license` / `pretraining_dataset_license` / `safetensors_available`
- **GPU 要件**：必要メモリ・fallback 可否

## ③ 出力形式
- **学習済みモデル**：`safetensors` を優先、`torch.save` の場合は `weights_only=True` 可読
...
- **不確かさ閾値**：<predictive entropy ≤ Z（エージェント自律停止条件）>
...
- **vol-03 Layer 1 (GPU)**：`gpu_backend` / `cuda_version` / `cudnn_deterministic` / `cudnn_benchmark` / `torch_deterministic_algorithms` / `cublas_workspace_config` / `random_seed_per_worker`
- **vol-03 Layer 2 (重み)**：`weights_uri` / `revision` / `weights_sha256` / `weights_license` / `pretraining_dataset_license` / `safetensors_available`
- **vol-03 Layer 3 (学習/推論)**：`finetune_config` / `augmentation_config` / `tolerance` / `checkpoint_overwrite_policy`

## ⑦ Agentic 権限（§4.7）
- `agent_authorization.level`：L1 / L2 / L3
- `training_job_approval`：承認者・承認済み HP 範囲
