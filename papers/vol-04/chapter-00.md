...
flowchart LR
    ...
    MCP["MCP サーバ群<br/>（Jupyter / arXiv / 自作 causal wrapper ...）"]
    Skill["Skill<br/>（再利用可能な分析手順）"]
    Libs["Python ライブラリ<br/>（DoWhy / EconML / causal-learn / PyMC / ...）"]
...
> [!NOTE]
> **MCP とライブラリの境界**：DoWhy / EconML / causal-learn などは通常の **Python ライブラリ** で、Skill から直接 `import` して使います（MCP サーバではありません）。組織方針として「エージェントが直接ライブラリを呼ぶことを制限し、承認ゲート付きの MCP 経由でしか呼べないようにしたい」場合に、**自作 MCP（causal wrapper）** でラップします——実装パターンは付録B。
...
| **scikit-uplift** | 第8章 | uplift modeling（材料選抜への転用。**binary treatment / binary outcome 中心。連続処置には自作拡張が必要**） |
| **pyDOE2** | 第10-11章 | 古典 DoE（要因計画・応答曲面。**split-plot / 複雑な blocking は自作補完が必要**） |
...
| **positivity / SUTVA / consistency / exchangeability の宣言** | 因果的識別に必要な 4 仮定（positivity=処置群/対照群のオーバーラップ、SUTVA=介入の相互干渉なし、consistency=観測処置と反実仮想処置の一致、exchangeability=未観測交絡なし）を Skill が明示的に `declared / assessed` で記録。**checked と主張するのは positivity のみ**（他は反証可能性が限定的） |
...
| **反実仮想の外挿範囲** | `counterfactual_scope_gate`——外挿範囲を Mahalanobis 距離閾値 + CATE 予測分散閾値で判定（**連続共変量向け。カテゴリ変数・画像・スペクトル特徴では embedding 距離や近傍密度を併用**） |

### 品質チェックは 3 段階（vol-01 の哲学、vol-02〜04 で継承）

| レベル | 挙動 | vol-04 での新たな例 |
|---|---|---|
| **fatal** | Skill に渡さず**明示エラーで拒否** | **collider を調整変数に含めて backdoor path を開いている**、**backdoor 基準を満たさない adjustment set**、**positivity 違反（treatment/control のオーバーラップが不十分）**、**randomization seed の欠落** |
...
| フィールド | 意味 |
|---|---|
| `causal_graph_uri` | 使用した DAG 定義ファイルの URI（**Ch4 §4.4 で canonical `dag_of_record_uri` に統一。本節の呼称は初期 draft の alias**） |
| `causal_graph_sha256` | DAG 定義ファイルの SHA-256（改ざん防止、canonical: `dag_of_record_sha256`） |
...
| `counterfactual_scope_gate` | 反実仮想の外挿範囲判定（連続共変量では Mahalanobis 距離閾値 + CATE 予測分散閾値、カテゴリ・画像・スペクトル特徴では embedding 距離や近傍密度を併用） |
...
**（A）3 層 Intervention Authorization Gate**（v0.2 で `intervention_authorization` を 3 分解、**因果的判断そのものへの権限**）
...
**（B）DoE / Validation / Safety Gate**（**因果的判断ではなく、実験実施の安全性と結論の妥当性への関門**）
...
| **refutation pass 未達時の結論停止** | 第9章 | E-value・Rosenbaum bounds・placebo test が pass しない場合、Skill が結論を出さない契約。**pass は反証シナリオに耐えたことを示すのみで、因果効果の証明ではない** |
...
**因果推論一般（第14章 セクション 1）**：DAG の misspecification、collider を adjustment に含めることによる bias、positivity violation、consistency / exchangeability 仮定の暗黙化、外挿の unwarranted、CATE の過剰個別化、refutation スキップ、**frontdoor / RDD / synthetic control の適用条件を満たさないまま採用**。ARIM 材料実験では **backdoor 調整 / DR / DiD / IV が主戦場**で、frontdoor・RDD は条件が揃う場合に限定されます。
...
**(4) 反実仮想の外挿範囲（Counterfactual Scope Gate）**：「もし温度を 50 度上げていたらどうなったか」といった反実仮想は、観測データの範囲を越えた瞬間に不確定になります。**vol-04 では反実仮想の外挿範囲を Mahalanobis 距離閾値 + CATE 予測分散閾値で判定する `counterfactual_scope_gate` を Skill に組み込みます**（第8-9章）。連続共変量が主対象で、**カテゴリ・画像・スペクトル特徴では embedding 距離や近傍密度を併用**します。エージェントは範囲外で "自信あり" の予測を返してはいけません。
