...
> [!IMPORTANT]
> **canonical identity schema（共通、v0.2 拡張）**：本節（§4.6.2）で定義される承認記録は、vol-03 `training_job_approval` / vol-04 L1-L4 authorizations / vol-05 `experiment_launch_authorization` と共有する canonical identity schema に従う。3 volume で機械照合可能とするため、次の 3 フィールドが canonical：
>
> - `authorization_id`：canonical 形式 `l{n}_auth_YYYYMMDD_HHMMSS_iter<n>`（L3 の場合 `l3_auth_20261121_102345_iter7` 等）。vol-05 の `parent_authorization_id: "vol-04:L3_intervention_execution_authorization:<authorization_id>"` から機械照合可能。
> - `status`：enum `pending | approved | denied | revoked`（4 値のみ canonical）。approval_timestamp と組で使用。
> - `approver_id`：canonical regex `^human:` に一致する文字列（例：`human:staff_id_XXXX`）。vol-05 §5.3 の `writer_identity_restriction: "^human:"` と同一スキーマ。既存の `approver` フィールド（role token: `research_lead` / `pi_and_facility_manager` / `facility_causal_review_board`）は v0.1 compat として残置し、role token + individual identity の両方を記録する。
>
> **v0.1 → v0.2 migration**：既存 record（`approver` role token のみ）は v0.1 として扱い、新規 record は v0.2（`authorization_id` + `status` + `approver_id` を必須）で書く。vol-03/04/05 の共通 identity schema は本書全体の shared appendix として本節を SoT とする。
...
authorization_gates:
  dag_authorization:
    ...
    approver: research_lead              # デフォルト（個別 PI-scope の判断）— v0.1 compat（role token）
    approver_id: "human:<staff_id>"      # v0.2 canonical identity（regex ^human:）
    authorization_id: "l1_auth_YYYYMMDD_HHMMSS_iter<n>"   # v0.2 canonical id
    status: pending                       # v0.2 canonical enum: pending | approved | denied | revoked
  ...
  variable_selection_authorization:
    ...
    approver: research_lead              # デフォルト（個別 PI-scope）— v0.1 compat（role token）
    approver_id: "human:<staff_id>"      # v0.2 canonical identity（regex ^human:）
    authorization_id: "l2_auth_YYYYMMDD_HHMMSS_iter<n>"   # v0.2 canonical id
    status: pending                       # v0.2 canonical enum: pending | approved | denied | revoked
  ...
  intervention_execution_authorization:
    ...
    approver: pi_and_facility_manager     # v0.1 compat（role token）
    approver_id: "human:<staff_id>"       # v0.2 canonical identity（regex ^human:、vol-05 §5.3 と共通）
    authorization_id: "l3_auth_YYYYMMDD_HHMMSS_iter<n>"   # v0.2 canonical id (vol-05 parent_authorization_id 参照先)
    status: pending                        # v0.2 canonical enum: pending | approved | denied | revoked
    approval_scope:                        # v0.2 canonical — candidate 単位で承認 scope を pin
      candidate_hash: "sha256:..."         # 対象 candidate の hash（vol-05 §5.3 constraint_precheck と揃える）
      experiment_id: <string>              # 実験の canonical id
      valid_until: <timestamp>             # scope の有効期限（超過後は re-auth 必須）
  ...
  approver_independence:
    ...
    approver_id_regex: "^human:"           # v0.2 canonical — vol-03/04/05 共通 identity schema
    ...
    facility_scope_escalation:
      ...
      default_approver_id: "human:<facility_reviewer_id>"   # v0.2 canonical identity
      default_authorization_id: "l4_auth_YYYYMMDD_HHMMSS_iter<n>"  # v0.2 canonical id
...
### 4.6.5 承認スキーマ・クロスリファレンス表（vol-03 / vol-04 / vol-05）

> [!IMPORTANT]
> 同一 概念（approval record / approver identity / status）に対する field 名の drift を可視化する。今後の Skill 実装は本表 v0.2 の canonical name（`authorization_id`, `status`, `approver_id`）を優先し、v0.1 compat フィールド（`approver` / `approver_role` 等）は継続保持する。

| 概念 | vol-03 (Ch7 / appendix-a training_job_approval) | vol-04 (§4.6.2 L1-L4 authorizations) | vol-05 (Ch5 §5.3 experiment_launch_authorization) |
|:---|:---|:---|:---|
| 承認記録の canonical id | `approval_record_id` (sha256) | `authorization_id`（v0.2、`l{n}_auth_YYYYMMDD_HHMMSS_iter<n>`） | `authorization_id`（`elauth_YYYYMMDD_HHMMSS_iter<n>`） |
| 状態 enum | `required: true/false` (bool) | `status` v0.2: `pending \| approved \| denied \| revoked` | `status`: `pending \| approved \| denied \| revoked` |
| 承認者 identity（v0.1 compat） | `approver_identity`, `approver_role`, `approver_is_non_agent` | `approver`（role token: `research_lead` \| `pi_and_facility_manager` \| `facility_causal_review_board`） | （v0.2 で新設、role tokenは v0.1 compat） |
| 承認者 identity（v0.2 canonical） | 未実装（self-contained gate、design note 参照） | `approver_id`: `^human:<staff_id>` | `approver_id`: `^human:<staff_id>` |
| 承認範囲 | `approved_hp_range: {lr, batch_size, epochs_max}` | `approval_scope: {candidate_hash, experiment_id, valid_until}` | `approval_scope: {iteration_index, candidate_hash, batch_size, max_experiments_authorized}` |
| 親 authorization | `parent_authorization_id: null`（self-contained、design note） | 該当なし（vol-04 が最上位 gate 群） | `parent_authorization_id: "vol-04:L3_intervention_execution_authorization:<authorization_id>"` |
| 承認 event stream | checkpoint registry (v0.2 で optional `previous_event_hash`) | `agent_action_log.gate_interaction_events` (v0.2 で `event_hash` chain 対応) | `authorization_events_stream`, `experiment_launch_authorization_log`（Ch5 §5.3、Ch10 §10.5 schema） |
| 承認者 identity regex | 未指定（`approver_is_non_agent: true` のみ） | `approver_id_regex: "^human:"`（§4.6.2） | `writer_identity_restriction: "^human:"`（Ch5 §5.3 / Ch15 §15.3） |
| audit 検証責務 | (書換不可 + `checkpoint_overwrite_policy: append_only`) | Ch14 §14.4 audit_manifest_v1 (19 detection checks) | Ch15 §15.3 audit（event chain replay + candidate_hash 一致） |

**canonical identity schema の共有**：v0.2 以降、`authorization_id` + `status` + `approver_id: ^human:` の 3 フィールドが 3 volume 共通の canonical identity schema。vol-03 は self-contained gate として `parent_authorization_id: null` を明示するが、他 3 フィールドは今後の v0.3 で align 予定。
...
skill:
  ...
  # === ⑨ Agent action log（Ch14 §14.3.3 で back-register、v0.2 拡張で event_hash chain 対応） ===
  agent_action_log:                              # Skill が発火した gate 相互作用の監査ログ
    gate_interaction_events:                     # 各 event は list of dict
      - gate_id: <string>                        # e.g., counterfactual_scope_gate | refutation_gate | L1_dag_authorization
        event_id: <string>                       # v0.2: 各 event の canonical id
        event_hash: <string>                     # v0.2: sha256(canonical(payload)), vol-05 Ch10 §10.5 event_canonical_serialization 準拠
        previous_event_hash: <string>            # v0.2: 直前 event の event_hash（改ざん検知チェーン、genesis は null）
        run_id: <string>                         # v0.2: run 単位の canonical id
        skill_execution_id: <string>             # v0.2: skill 実行単位の canonical id
        attempt_result: pass | fail | bypass_attempted | not_applicable
        timestamp: <timestamp>
        suppressed: <bool>                        # Skill が result を suppress したか（bypass detection 用）
    log_uri: <string>
    log_sha256: <string>                          # v0.1 file-level integrity（compat）
    log_append_only: true                        # immutability（改竄検出）
    event_hash_schema_ref: "vol-05:ch10:event_canonical_serialization"  # v0.2 canonical: per-event hash chain schema
    # v0.1 → v0.2 migration: v0.1 では log_sha256 のみ（file-level tamper detection）。
    # v0.2 では追加で per-event の event_hash / previous_event_hash chain を採用し、
    # 個別 event 単位で改ざん検知可能とする（vol-05 Ch10 §10.5 の schema を共通使用）。
