...
| 16 | Post-treatment variable の adjustment（Ch5 §5.2.5、Ch14 §14.1.1） | **fatal** | `adjust_for_post_treatment_variable_without_marking_as_mediator` — DAG の temporal ordering チェックで adjustment set 内の変数が処置後に測定されているのに mediator 宣言がない | `variable_selection_authorization` | — |
...
| 21 | 監査 skill が全 detection check を走らせずに pass 主張（Ch14 §14.5.1） | **fatal** | `claim_audit_pass_without_running_all_checks` — `audit_result_summary.pass_count + fail_count + not_applicable_count ≠ total_checks_expected` | `facility_standard_promotion_gate`（L4） | — |
