...
エージェントは「どのモデルで応答を近似するか」を **一気に決めない**——3 段階の階層で提案し、各段で Ch4 §4.6.2 の approver に承認を求めます：

$$
\underbrace{y = \beta_0 + \sum \beta_i x_i + \sum \beta_{ij} x_i x_j + \sum \beta_{ii} x_i^2 + \varepsilon}_{\text{L1: 2 次多項式}}
\;\longrightarrow\;
\underbrace{y = f_{\text{GBM}}(x) + \varepsilon}_{\text{L2: 非線形}}
\;\longrightarrow\;
\underbrace{y = f_{\text{GP}}(x) + \varepsilon,\; \varepsilon \sim \mathcal{GP}(0, k)}_{\text{L3: 不確かさ付き surrogate}}
$$

**契約含意**：

- **L1 → L2 昇格**：`estimator_contract_change_gate`（Ch6 §6.7）L2 escalation、`research_lead` 承認
- **L2 → L3 昇格**：L3 承認 + `counterfactual_scope_gate` の再検証（GP は補外域で不確かさが爆発するため）
...
| 種類 | $\alpha$ の値 | 特性 |
|---|---|---|
| **Rotatable** | $\alpha = (2^k)^{1/4}$（$k=3$ で $\approx 1.682$）| 全方向で予測分散が等しい（推奨） |
...
experimental_design_provenance:
  ...
  axial_component:
    n_runs: 6
    alpha_value: 1.682                                 # $(2^k)^{1/4}$ for k=3
...
エージェントは応答曲面フィッティングを **1 発でモデル選択しない**——3 段階で spec を書き、各段で承認を求めます：

| 段階 | モデル | 使うタイミング | ライブラリ |
|---|---|---|---|
| **L1** | 2 次多項式回帰 | curvature test 有意、変数 3-5 個、runs 15-30 | statsmodels.ols |
| **L2** | GBM（LightGBM/XGBoost）| L1 の residual に非線形パターン、runs 30+ | lightgbm |
| **L3** | GP（Kriging）| **予測不確かさ**が必要（Bayesian DoE 引継ぎ、外挿検知）| smt.surrogate_models.KRG |
...
```yaml
skill_type: response_surface_model_selection_skill
authorization_level: agent_autonomous_with_l2_gate
inputs:
  - design_matrix_uri
...
outputs:
  - model_selection_report_uri
  - selected_model: <one_of_candidate_models>
  - selection_rationale:
      curvature_test_p_value: <float>
      residual_diagnostics_uri: <string>               # L1 の residual プロット
      candidate_model_cv_scores: dict                  # 各候補モデルの CV RMSE
  - counterfactual_scope_gate_result: <object>          # 実験域外への予測要求チェック
...
authorization_gate:
  # L1 (2 次多項式) は agent 自律、L2 以上は estimator_contract_change_gate
  l1_to_l2_upgrade:
    required_approver: research_lead
    required_evidence: [residual_diagnostics_uri, cv_scores_comparison]
  l2_to_l3_upgrade:
    required_approver: research_lead
    required_evidence: [residual_diagnostics_uri, cv_scores_comparison, extrapolation_use_case_declaration]
```
...
L3 の Kriging / GP は、点推定 $\hat{y}(x)$ に加えて **予測分散 $\hat{\sigma}^2(x)$** を返します。応答曲面設計では、これが 3 つの用途で **必須**：
...
authorization_level: agent_autonomous_with_l3_gate
...
prohibited_actions:
  ...
  - collapse_variance_to_zero_at_training_points_in_output_ci      # warning（GP は training で分散 0 だが、CI 出力時は必ず記録）
```
...
| **L18** | 18 | 1 × 3-level + 7 × 3-level | 混合水準 | 混合水準 screening + optimization |
...
- `optimum_on_design_boundary: true | false`
...
skill:
  ...
  # --- counterfactual scope gate (§11.7.2) ---
  counterfactual_scope_gate:
    mahalanobis_check: {...}
    variance_check: {method: gp_predictive_variance, ...}
    knn_density_check: {k: 5, knn_min: 2}
    support_envelope_check: {strict: true, ...}
    aggregate_policy:
      pass_requires: all_four_pass
      conditional_pass_output: non_actionable_diagnostic_only
      fail_action: fail_close_and_reject_optimum_recommendation
    ...
    approver_independence:
      ...
      facility_scope_escalation:
        applies_to:
          ...
          - approved_facility_response_surface_baseline    # 応答曲面 baseline の facility 標準化
...
level_maps = {
    "temperature": {-1.682: 350, -1: 400, 0: 500, 1: 600, 1.682: 650},
    "pressure":    {-1.682: 0.3, -1: 1.0, 0: 5.0, 1: 10.0, 1.682: 12.0},
    "pretreatment_time": {-1.682: 5,  -1: 15,  0: 22,  1: 30,  1.682: 35},
}
# note: 実運用では coded → actual mapping を YAML で pin
...
- [ ] **§11.1.1 モデル階層 L1/L2/L3**（linear → GBM → GP）と、L2 / L3 昇格時の `estimator_contract_change_gate` 承認要件を書ける
...
- **第6章**：`estimator_contract_change_gate` L1-L4（本章 §11.1.1 モデル階層昇格に適用）
