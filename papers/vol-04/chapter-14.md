...
**原因**：Ch10 §10.5.3 の 4-stage assignment_log detection（`seed_match` / `design_hash_match` / `permutation_reproducibility` / `execution_records_binding`）のいずれかが fail。
...
detection_check:
  ...
  fatal_action_referenced:
    ...
    - Ch11.interpret_taguchi_sn_as_pure_variance_index_without_loss_function          # Ch11 §11.8 back-register 済み
...
**原因**：Ch4 §4.6.1 の DAG 承認契約と Ch5 §5.6 の Skill 分離が不完全に実装され、`skill_1a_dag_proposal` の出力を `skill_2a_variable_selection` が **無断で改変** できる。
...
**原因**：Skill の autonomy 境界が曖昧。Ch4 §4.6.1 の `dag_authorization` は DAG 全体を承認するが、adjustment set の具体的な列選択を **Skill 側の autonomy** に委ねている実装で発生。
...
detection_check:
  ...
  automated_checks:
    - approved_covariates_pin_check:           # variable_selection_authorization で pin
        approved_covariates: <list>             # Ch4 §4.6.2 canonical (Table 4.4 item 17)
    ...
    - autonomy_boundary_check:                 # Ch4 §4.6.3 canonical
        skill_action_class: <declared>
        actual_action: <observed>
        boundary_violation: <bool>
        fail_action_if_true: fatal_agent_autonomous_covariate_removal
  fatal_action_referenced:
    - Ch4.Table_4_4_item_5                                       # Ch4 §4.8 item 5 (Refutation skip — declared_required_tests)
    - Ch4.Table_4_4_item_9                                       # Ch4 §4.8 item 9 (Unauthorized intervention execution — intervention_execution_authorization)
...
**原因**：Skill の action_class が `propose_only` ではなく `propose_and_execute` の場合、エージェントが gate 自体を bypass する誘惑にさらされる。Ch9 §9.7 の canonical では `refutation_gate` は L2（variable_selection_authorization）の前提条件だが、Skill 実装で強制されないと skip される。**Skill の意図的 bypass 記録は `agent_action_log`（Ch4 §4.9 template ⑨ に back-register 済み）から取得。**（N-5 修正）
...
**原因**：Ch4 §4.6.2 の `intervention_execution_authorization` の temporal ordering 契約が不完全。Skill が「介入を先に実施して事後承認を取る」パターンで実装される（fail-forward）。**Ch4 §4.6.2 に本節で back-register 済み**（`intervention_execution_timestamp` + `temporal_ordering` block）。（B-3 note、N-11）
...
detection_check:
  ...
  fatal_action_referenced:
    ...
    - Ch11.execute_with_seed_mismatch_between_pin_and_execution     # Ch11 §11.8 back-register 済み
```
...
**原因**：Ch4 §4.6.2 の L3 authorization が strict でなく、Skill の action_class が `propose_and_broadcast` に近い実装。**送信先チャネル宣言（`egress_channels_declared`）と `authorized_broadcast_targets` は Ch4 §4.9 template ⑩ egress_control に back-register 済み**（N-6 修正、N-11）。
...
```yaml
skill_contract:                                 # Ch4 §4.9 canonical に準拠
  ...
  gate_level: L4_facility_standard_promotion   # Ch4 §4.6.2 canonical
  input_provenance:
...
prohibited_actions:
  ...
  #       (Ch11 §11.8 に misdeclare_sn_ratio_type / switch_sn_ratio_type_after_execution /
  #        interpret_taguchi_sn_as_pure_variance_index_without_loss_function /
  #        report_sn_only_without_mean_variance_separation の 4 fatal を back-register 済み — N-7)
...
- [ ] `silent_confounder_removal_check` が Ch4 §4.6.2 の approved_covariates と実 estimator input の symmetric difference を検査する
...
| 大規模 LLM / foundation model の agent 委譲（tool use、multi-agent orchestration） | 対象外（vol-05 に完全委譲、N-14） | **vol-05** |
