...
$$
\alpha_{\text{EI}}(x) = \mathbb{E}\bigl[\max(0,\ y^* - \mu(x))\bigr]
= (y^* - \mu(x))\Phi(z) + \sigma(x)\phi(z)
$$

ここで $y^*$ は現在の best、$z = (y^* - \mu(x)) / \sigma(x)$（**最小化問題**の場合）。最大化なら符号を反転。
...
$$
\alpha_{\text{PI}}(x) = \Phi\!\left(\tfrac{y^* - \mu(x) - \xi}{\sigma(x)}\right)
$$
...
$$
\alpha_{\text{MES}}(x) = H\bigl[p(y^* | \mathcal{D}_{1:t})\bigr] - \mathbb{E}\bigl[H[p(y^* | \mathcal{D}_{1:t}, (x, y))]\bigr]
$$
...
[Q3] Batch（q 個同時提案）にする？
  ├─ YES → qLogEI / qUCB / qKG / qMES / Thompson Sampling（第12章）
  └─ NO  → 通常版
...
acquisition_spec:
  # --- 必須フィールド ---
  name: "qLogExpectedImprovement"  # enum: qLogEI | qLogNoisyEI | qUCB | qPI | qKG | qMES | qTS | qPES | cEI | qEHVI | ...
  objective_direction: "maximize"  # maximize | minimize
  
  # --- optimizer 設定（数値最適化の hyperparameter）---
  optimizer:
  ...
  hyperparameters:
    # UCB の場合: beta
    # KG / MES の場合: num_fantasies
    # PI の場合: xi
    # ここに必要な hyperparameter を平坦に列挙
...
```yaml
acquisition_spec:
  name: "qLogExpectedImprovement"  # 初期 acquisition
  dynamic_switching:
    allowed: true
    allowed_transitions:
      - from: "qLogExpectedImprovement"
        to: "qUpperConfidenceBound"
        trigger:
...
acquisition_switch_events:
  - from_iteration: 5
    to_iteration: 6
    from_acquisition: "qLogExpectedImprovement"
    to_acquisition: "qUpperConfidenceBound"
...
- [ ] Switch 時にも `experiment_launch_authorization` は毎回必要（§5.3）
...
### 7.7.3 判定 3：Length scale 越え（$L_{\text{ratio}}$）

- **やること**：候補 $x$ の bounds からの距離を length scale と比較
- **式**：$L_{\text{ratio}}(x) = \max_i \bigl|(x_i - \tfrac{1}{2}(b_i^{\text{low}} + b_i^{\text{high}}))\bigr| / \ell_i$
  - $b_i^{\text{low/high}}$ は次元 $i$ の bounds
- **推奨閾値**：$\tau_L = 1.5$（探索空間中心から length scale の 1.5 倍以上離れたら flag）
- **意味**：「探索空間の端の外挿領域」を検出
...
def hallucinated_recommendation_detection(x_cand, model, X_obs, bounds,
    ...
    # 判定 2: Mahalanobis 距離（length scale で正規化）
    lengthscale = model.covar_module.base_kernel.lengthscale.detach().squeeze()
    diffs = (x_cand.unsqueeze(0) - X_obs) / lengthscale  # (N, d)
    D_Mah = diffs.pow(2).sum(dim=-1).sqrt().min().item()
    
    # 判定 3: length scale 越え
    center = (bounds[0] + bounds[1]) / 2
    L_ratio = ((x_cand - center).abs() / lengthscale).max().item()
  ...
  operational_definition:
    ...
    criteria:
      ...
      - name: "mahalanobis_distance"
        formula: "min_i sqrt((x_cand - x_i)^T diag(1/ell^2) (x_cand - x_i))"
        threshold: 3.0
      - name: "length_scale_ratio"
        formula: "max_i |x_cand_i - center_i| / ell_i"
        threshold: 1.5
