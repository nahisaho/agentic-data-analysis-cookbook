...
search_space_bounds:
  ...
  bo_applicability:
    ...
    rule_applied: 4
  provenance_backreference:
    vol04_dag_hash: "sha256:aaaa..."         # vol-04 で承認された DAG の hash
...
surrogate:
  version: "v0.1"
  model_family: "single_task_gp"             # Ch5 canonical
  kernel_spec:
  ...
  noise_model: "fixed_noise_gp"               # 実験ノイズが既知（yield 測定は再現性 2-3%）
...
stop_condition:
  ...
  clauses:                                      # AND 結合
    ...
    - {name: "budget",        condition: "budget_remaining.experiments_remaining <= 5",  reason: "confirmation reserve 保護"}
...
surrogate:
  ...
  hrd_ref:
    ...
    scale_source:
      ...
      exclude_variables: ["facility_id"]        # task index は距離から除外（Ch11）
```
...
stop_condition:
  ...
  clauses:
    ...
    - {name: "task_diagnostic",    condition: "task_correlation_matrix.tilde_B_normalized diagnostic in [all_ones, identity]",  reason: "階層 GP の pooling 失敗"}
    - {name: "human_stop",         condition: "human_stop_signal == true",                                       reason: "Human override"}
...
Iteration 6-10 の実装スクリプトは **付録 D** に収録。以下を扱う：

- BoTorch `qLogNoisyExpectedHypervolumeImprovement` の Multi-task 版
...
1. **Priority**：hard_constraints > pending_experiments > acquisition > batch_diversity_policy > stop_condition。上位が下位を上書きしない
2. **Ref-only**：他章契約は `_ref` 参照で示す（`hrd_ref`, `experiment_launch_authorization_ref`）。inline copy 禁止（drift の温床）
3. **Version pin**：iteration 開始時に全契約の version を snapshot（`contract_version_pin` として run_id と紐付け）
4. **Approval scope match**：`experiment_launch_authorization.scope` が `candidates` と bijective
5. **Machine-verified precheck**：Ch10 Layer 3 constraint_precheck が全 candidate に対して pass
6. **Idempotent iteration**：同じ input で iteration を再実行すると同じ candidates が出る（seed pin、Ch15 §15.x）
...
- [ ] Phase 1：`search_space_bounds.derived_from` に vol-04 因果 DAG の hash + approval_id
- [ ] `linear_constraints` が DAG 由来で明示
- [ ] Phase 2：14a は `single_task_gp`、14b は `hierarchical_gp` に切替。**同一 Skill で兼務しない**（別 Skill 登録）
...
- [ ] `batch_diversity_policy.enforcement: post_hoc_skill_validation`（Ch12）
...
- [ ] `experiment_launch_authorization.scope` が candidates と bijective
- [ ] `budget_remaining` と `stop_condition` が毎 iteration 更新、Skill 自律緩和禁止
- [ ] `contract_version_pin` を iteration 開始時に snapshot、run_id と紐付け
- [ ] `task_correlation_matrix` を毎 iteration 記録、degeneracy 診断で自律停止
- [ ] `hierarchical_model_degeneracy_alert` の発行フロー明確
- [ ] 14a → 14b の Skill 切替（single_task_gp → hierarchical_gp）は Human 承認付き
