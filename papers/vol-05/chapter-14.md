...
<a id="dag_revision_policy"></a>

### 14.4.0 `dag_revision_policy`（canonical）

> [!IMPORTANT]
> **canonical anchor**：`vol-05:ch14:dag_revision_policy`
>
> §14.2.1 Phase 1 で inline に宣言する `dag_revision_policy` の formal 仕様。Ch15 §15.3 audit が参照する SoT。
> DAG（`dag_of_record`）は run 内 immutable。差分は必ず新 SHA256 + human 承認による re-derive を要する。

```yaml
dag_revision_policy:                              # canonical schema
  version: "v0.1"
  immutable_within_run: true                      # bool: run 内での DAG 書換は fatal
  on_dag_hash_mismatch:                            # enum
    value: "fatal_new_run_required"
    choices: ["fatal_new_run_required", "warn_and_continue", "auto_rederive"]
    canonical: "fatal_new_run_required"           # 本書 canonical（他 2 値は非推奨）
  on_new_approved_dag_available:                   # enum
    value: "continue_current_run_with_pinned_dag"
    choices: ["continue_current_run_with_pinned_dag", "escalate_for_re_run"]
  requires_human_approval_for_rederive: true       # bool: DAG re-derive は必ず HITL（vol-04 L1）
  rederive_authorization_gate: "vol-04:L1_dag_authorization"
  audit_event_stream: "dag_revision_events"        # 遷移 event stream 名（下記）

dag_revision_events:                              # append-only, Ch10 §10.5 event_hash schema
  - event_id: "evt-dagrev-20261210-001"
    event_hash: "sha256:..."
    ...
    payload:
      transition: "propose_rederive"              # enum: propose_rederive | approved | rejected | pinned
      dag_of_record_uri_before: "vol-04:appendix-c:causal_dag_ceramic_v1"
      dag_of_record_sha256_before: "sha256:aaaa..."
      dag_of_record_uri_after: null
      dag_of_record_sha256_after: null
      rationale: "施設 B の新規変数追加提案"
      parent_authorization_id: "vol-04:L1_dag_authorization:l1_auth_20261210_075500"
```

**canonical rules**：

- `immutable_within_run: true` が canonical（false は非推奨、fatal に近い運用）
- `on_dag_hash_mismatch: "fatal_new_run_required"` 以外を選ぶ場合、`facility_causal_review_board` の追加承認を要する
- `dag_revision_events` の writer identity は `^human:`（DAG 変更提案は必ず human 起点）

<a id="experiment_reservation_events"></a>

### 14.4.0-b `experiment_reservation_events`（canonical）

> [!IMPORTANT]
> **canonical anchor**：`vol-05:ch14:experiment_reservation_events`
>
> 装置予約枠（`reserved_for_confirmation`）の割当と解放を event として記録。Ch15 §15.3 audit の資源整合性検証の SoT。

```yaml
experiment_reservation_events:                    # append-only, Ch10 §10.5 event_hash schema
  - event_id: "evt-resv-20261211-001"
    event_hash: "sha256:..."
    previous_event_hash: "sha256:..."
    run_id: "run-2026-1210-04"
    skill_execution_id: "skill-exec-XYZ"
    created_at: "2026-12-11T08:00:00Z"
    created_by: "skill:experiment_orchestrator"   # 装置予約は Skill 自動、ただし解放は HITL 可
    payload:
      reservation_id: "resv_dev_A_20261211_090000"
      facility_id: "A"
      transition: "reserved"                       # enum: reserved | released | consumed | cancelled | expired
      slot_start: "2026-12-11T09:00:00Z"
      slot_end:   "2026-12-11T17:00:00Z"
      reservation_purpose: "reserved_for_confirmation"   # enum: reserved_for_confirmation | dedicated_experiment | maintenance
      linked_candidate_id: "cand-A-041"
      linked_experiment_id: null                    # まだ launch 前
      authorization_id: null                        # L3 前段の暫定予約
  - event_id: "evt-resv-20261211-002"
    event_hash: "sha256:..."
    previous_event_hash: "sha256:..."
    run_id: "run-2026-1210-04"
    skill_execution_id: "skill-exec-XYZ"
    created_at: "2026-12-11T09:00:15Z"
    created_by: "skill:experiment_orchestrator"
    payload:
      reservation_id: "resv_dev_A_20261211_090000"
      transition: "consumed"
      linked_experiment_id: "exp-A-041"
      authorization_id: "approval:HITL-20261211-A41"
```

**canonical rules**：

- transition enum: `reserved | released | consumed | cancelled | expired`（consumed は experiment_launch と bind）
- 未使用時間切れの reservation は `expired` として append（Skill 自動）
- `reserved_for_confirmation` の解放は HITL 可（`created_by: ^human:`）、Skill 自動での解放は fatal（Ch12 §12.7 cancellation と同種）
