...
> **本章の到達目標**
> - 第4章の環境を使い、**自然言語だけで**データ分析の一連の流れ（読込→可視化→特徴抽出→考察）を動かせる
...
第4章で「動作確認済み環境」を手に入れました。本章ではそれを使い、**自然言語で指示するだけで分析が進む**ことを体験します。ゴールは1つ、次の状態です。

> **AI Agent に日本語で指示するだけで、データの読込・可視化・特徴抽出・考察までが1つのノートブックの上で進む。**
...
- **ターミナル1**：`arim-analysis` ディレクトリで venv を有効化し、`jupyter lab --port 8888 --IdentityProvider.token MY_TOKEN` を起動したままにする。
...
```text
● Use Notebook (MCP: jupyter) · notebook_path: "analysis.ipynb"
● Insert and Execute Code Cell (MCP: jupyter)
...
AI Agent がプロット用セルを追加・実行し、グラフ画像が返ってきます（第4章で `ALLOW_IMG_OUTPUT=true` にしたため、画像を受け取れます）。ピークの当たりが目で確認できるはずです。
...
> このスペクトルのピークを検出して、それぞれの波数（ピーク位置）と強度を表にしてください。
  scipy.signal.find_peaks を使い、prominence=0.1、distance=20（サンプル）で検出してください。
...
> 検出結果と、ステップ1で保存した true_peaks を並べた比較表を作ってください。
  検出ピーク数・位置が true_peaks と一致しているかを確認し、最初の問い（何本のピークが、どの位置にあるか）に答えてください。
```

合成データなので**正解（`true_peaks` の値）**が分かっています。検出結果がそれと一致していれば、5ステップが正しく通ったことになります。これが本章の**成功条件**です。
...
> [!TIP]
> エラーが出たら、その内容をそのまま貼り付けて AI Agent に相談できます。ただし提案の妥当性は自分で確認してください（詳細な対処は付録C）。
...
- 第4章の環境を使い、**自然言語だけで**取得＋整形→可視化→ピーク検出→考察の4ステップ（第2章の5ステップに対応）を通せた
...
- [脚注1] Jupyter MCP Server（Datalayer）ツール一覧: https://jupyter-mcp-server.datalayer.tech/tools ／ リポジトリ: https://github.com/datalayer/jupyter-mcp-server
