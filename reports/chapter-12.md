    ...
    for key, (lo, hi) in RANGES.items():
        ...
        for v in vals:
            if not isinstance(v, (int, float)):
                errs.append(f"warning: {key} is not numeric: {v!r}")
                continue
            if v < lo:
...
| 手法 | 想定シナリオ | 判定ルール |
|---|---|---|
| **中央値絶対偏差（MAD）** | 繰り返し測定の 1 点が飛ぶ | `|x - median| / MAD > 3.5` を外れ値とみなす |
...
def mad_outliers(values: list[float], threshold: float = 3.5) -> list[int]:
    ...
    if mad == 0:
        return []  # ばらつき0：判定不能
    # 正規分布近似の補正係数 0.6745
    return [i for i, v in enumerate(values) if abs(v - med) / mad > threshold / 0.6745]
...
if __name__ == "__main__":
    ...
    for k, vs in data.items():
        idx = mad_outliers(vs)
        if idx:
            print(f"{k}: outliers at indices {idx} (values: {[vs[i] for i in idx]})")
```
...
| 層 | 「同じ」の意味 | 確認方法 |
|---|---|---|
| **L1 数値レベル** | 完全一致（浮動小数点誤差含む） | 同一環境・同一乱数種で2回実行し diff |
...
- [ ] **②入力条件**：データ契約 7 要素（+ マルチモーダルは⑧統合キー）を満たす
- [ ] **③出力形式**：`references/output-schema.json` があり、JSON Schema で valid
- [ ] **④成功条件**：数値で書ける（曖昧な "正しく動く" は NG）
- [ ] **⑤禁止事項**：**物質同定・帰属を含まない**（第9〜11章共通）
- [ ] **⑥再現性条件**：`skill_version`, `package_versions`, `random_seed` を記録
...
- [ ] **⑬既存手法との一致**：標準試料・手計算・別ツールの少なくとも 1 つで確認
...
3. **データ読み込みSkill**を書く（第8章、`.github/skills/<name>-loader/`）
4. **分析Skill**を書く（第9章、`.github/skills/<name>-analysis/`）
...
2. **外れ値**：`find_outliers.py` に対し、`[10, 10.1, 10.2, 10.1, 99.9]` を入力して 99.9 が外れ値として検出されることを確認しなさい。**さらに**、`[10, 10, 10, 10, 10]`（ばらつき 0）で判定不能になることも確認しなさい。
3. **再現性**：自分の Skill を kernel restart→run all で 2 回実行し、**L1（数値完全一致）** が取れるかを確認しなさい。取れなければ原因（乱数種未固定・セル間状態依存・外部依存など）を特定して修正しなさい。
...
- 外れ値検出は **MAD** を推奨（少数試行に強いロバスト統計）
- 再現性は **L1（数値）／L2（判断）／L3（環境）**の 3 層で確かめる
- 既存手法との一致は「手計算・標準試料・別ツール・公表値」から少なくとも 1 つ
